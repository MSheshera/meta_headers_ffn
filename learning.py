...
def train_def_model(train_x, train_y, verbose=True):
    ...
    default model, but with some arguments changed. Keeping this
    around for reproducibility.
    """
    ...
    mlp_clf = neural_network.MLPClassifier(
        ...
        max_iter=300, # Default. Makes sense for me.
...
def train_passed_model(train_x, train_y, hidden_units, alpha, verbose=False):
    """
    Train model with paramaters passed.
    """
    start_time = time.time()
    # Initialize classifier and train it.
    # Showing arguments which are relevant.
    mlp_clf = neural_network.MLPClassifier(
        hidden_layer_sizes=(hidden_units, ), # Tuning.
        activation='relu',
        solver='sgd',
        alpha=alpha, # Tuning.
        batch_size=400,
